...
function do_issuem_archives( $atts ) {
	...
	<div class="issuem_archives_shortcode">
		...
		foreach ( $archives as $archive => $issue_array ) {
			...
			<div id="issue-<?php echo esc_attr( $issue_array[0]->term_id ); ?>" class="issuem_archive issuem-archives-issue <?php echo esc_attr( $class ); ?>">
				...
				$width           = apply_filters( 'issuem_cover_image_width', $issuem_settings['cover_image_width'] );
				$height          = apply_filters( 'issuem_cover_image_height', $issuem_settings['cover_image_height'] );
		...
		if ( ! empty( $limit ) ) {
			...
			<div class="next_previous_archive_pagination">
				...
				if ( 0 === $offset && $limit < $archive_count ) { //Previous link only $results .='<div class="alignleft"><a href="' . esc_url( add_query_arg( 'paged' , $paged + 1, $url ) ) . '">' . __( 'Previous Archives' , 'issuem' ) . '</a></div>' ; } else if ( $offset>= $archive_count ) {
					//Next link only
					?>
					<div class="alignright"><a href="<?php echo esc_url( add_query_arg( 'paged', $paged + 1, $url ) ); ?>"> <?php esc_html_e( 'Next Archives', 'issuem' ); ?></a></div>
					<?php
				} else {
					//Next and Previous Links
					?>
					<div class="alignleft"><a href="<?php echo esc_url( add_query_arg( 'paged', $paged - 1, $url ) ); ?>"><?php esc_html_e( 'Previous Archives', 'issuem' ); ?></a></div>
					<div class="alignright"><a href="<?php echo esc_url( add_query_arg( 'paged', $paged + 1, $url ) ); ?>"><?php esc_html_e( 'Next Archives', 'issuem' ); ?></a></div>
					<?php
				}
				?>
